...
snapshot(id){let me=this.players.find(p=>p.id===id);return{time:this.time,round:this.round,players:this.players.filter(p=>!me||p.id===id||this.visible(me,p)).map(({input,_hits,wander,...p})=>p),food:this.food.filter(f=>!me||dist(f,me)<1100),event:this.event,nextEvent:this.nextEvent,shots:this.shots,fx:this.fx,decoys:this.decoys,top:[...this.players].sort((a,b)=>b.score-a.score).slice(0,5).map(p=>({id:p.id,name:p.name,score:p.score})),humans:this.players.filter(p=>!p.bot).length,bots:this.players.filter(p=>p.bot).length,lastTop:this.lastTop}}
}
