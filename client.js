...
$('start').onclick=async()=>{audioStart();$('notice').textContent='';$('start').disabled=true;try{let name=$('name').value.trim()||'Khách';controlMode=$('controlMode').value||'simple';try{localStorage.setItem('wild-name',name)}catch{}if(online){let res=await fetch('/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,species:selected,room:($('room').value||'WORLD1'),count:+$('count').value})});if(!res.ok)throw Error(await res.text());let j=await res.json();pid=j.id;source=new EventSource('/events?id='+encodeURIComponent(pid));source.onmessage=e=>{state=JSON.parse(e.data);me=state.players.find(p=>p.id===pid);let n=state.players.filter(p=>!p.bot).length;let el=$('onlineCount');if(el)el.textContent=n+' người online · World 1'};source.onerror=()=>{if(active)toast('Mất kết nối phòng. Đang thử nối lại…')};world=null}else{world=new World(+$('count').value);me=world.join(name,selected);pid=me.id;state=world.snapshot(pid)}active=true;audioStart();lastHP=0;lastXP=0;lastPickups=0;lastEventId=0;questIndex=0;questBase={pickups:0,kills:0,bestChain:0};cam=me?{x:me.x,y:me.y}:{x:1800,y:1800};$('lobby').style.display='none';$('hud').style.display='block';resetControls();toast(controlMode==='simple'?'Rê chuột · trái dùng chiêu 1/giữ để chạy · phải dùng chiêu 2 · đủ nộ bấm Space':'Chuột trái: chiêu 1/chạy · chuột phải: chiêu 2 · đủ 100% bấm Space để NỘ')}catch(e){$('notice').textContent='Không vào được phòng: '+e.message}finally{$('start').disabled=false}};
$('ultimate').onclick=()=>{if(active)input.e=true};
$('leave').onclick=()=>{aiming=false;resetControls();document.exitPointerLock?.();if(online&&pid)fetch('/leave',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:pid})}).catch(()=>{});if(source)source.close();active=false;state=null;me=null;world=null;input.l=input.r=false;$('lobby').style.display='flex';$('hud').style.display='none';$('death').style.display='none';$('help').style.display='none'};
...
TERRAIN.filter(o=>o.type!=='tree').forEach(drawTerrain);
